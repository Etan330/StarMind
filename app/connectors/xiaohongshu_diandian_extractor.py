...
# 提取所用 prompt 的版本号。每次实质改动 XIAOHONGSHU_DIANDIAN_PROMPT 时 bump，
# 提取成功后随 metadata 落入 RawSource（key: extract_prompt_version），便于回溯不同 prompt 的产出质量。
DIANDIAN_PROMPT_VERSION = "diandian_v1"
    ...
    async def extract_content(self, share_text: str, url: str = "", content_type: str = "note", timeout_seconds: int = 240, max_attempts: int = 2) -> DiandianExtractResult:
        ...
        try:
            for attempt in range(1, attempts_limit + 1):
                ...
                if content and not is_low_quality_extract(content):
                    return DiandianExtractResult(
                        url=url,
                        transcript=content,
                ...
                # 低质量回绝（无法访问/请登录/换个问题试试）或超时：保留重试，统一 error 口径。
                last_error = "low_quality_response" if content else "xiaohongshu_diandian_timeout"
